    ...
    def __init__(
        ...
        for idx in range(len(item_list)):
            ...
            if self.proxies and idx < len(self.proxies):
                connector = ProxyConnector.from_url(self.proxies.pop(0))
        ...
        for proxy in self.proxies:
            AmazonMonitor.proxies.append(proxy)
...
class AmazonMonitor(aiohttp.ClientSession):
    ...
    def check_proxy_cooldown(self):
        if self.proxies_on_cooldown:
            for time, ip in self.proxies_on_cooldown.items():
                if time.time() - time > 5:
                    self.proxies.append(ip)
                    del self.proxies[time]
                    log.debug(f'Proxy [{ip}] is cooled and has been added back to the pool')
    ...
    async def stock_check(self, queue: asyncio.Queue, future: asyncio.Future):
        ...
        while True:
            ...
            if self.delay == 0:
                    if AmazonMonitoringHandler.proxies:
                        old_proxy = self.connector.proxy_url.__str__()
                        new_proxy = self.proxies.pop(0)
                        self.proxies_on_cooldown.update({time.time(): old_proxy})
                        self.connector = ProxyConnector.from_url(new_proxy)
                        log.debug(f'{self.item}: Switching from [{old_proxy}] to [{new_proxy}]')
                        break
                    else:
                        await asyncio.sleep(1)
